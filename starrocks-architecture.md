...
3. [Component Descriptions](#3-component-descriptions)
   ...
   - 3.2 [BE Nodes (Backend)](#32-be-nodes-backend)
...
6. [Replication and High Availability](#6-replication-and-high-availability)
   ...
   - 6.3 [BE Failure and Recovery](#63-be-failure-and-recovery)
   - 6.4 [S3 HA](#64-s3-ha)
...
12. [Scalability Considerations](#12-scalability-considerations)
    ...
    - 12.2 [BE Horizontal Scaling](#122-be-horizontal-scaling)
...
| Per BE node (7 BEs) | ~8,267 rows/sec average (load distributed by FE) |
...
**Rowset accumulation constraint**: Routine Load fires sub-tasks on a configurable interval. If `max_batch_rows` is set too low relative to ingest rate, BE nodes accumulate many small rowsets per partition, degrading merge and query performance. This is the StarRocks equivalent of ClickHouse's "too many parts" problem. `max_batch_rows` and `desired_concurrent_number` must be tuned to produce rowsets large enough to avoid accumulation (see [Section 4.1](#41-kafka-ingest-path-routine-load) and [Section 8.3](#83-routine-load-small-file-accumulation)).
...
| Local storage | NVMe/SSD PersistentVolumes on BE nodes (data cache only — not primary storage) |
...
| Bucket count per partition | 21 (3× BE count = 3 × 7) — starting point | Open — see [Decision #8](#9-open-decisions) |

---

## 2. Architecture Overview

The architecture uses StarRocks shared-data mode: FE nodes provide metadata coordination and query routing; BE nodes provide compute and local NVMe data caching; S3 is the source of truth for all persistent data.

> **Improvement over ClickHouse**: The ClickHouse architecture requires three separate concerns — ClickHouse Keeper (3–5 pods), an ingest cluster (2 pods), and a cache cluster (14 pods) — totaling 19–21 pods. StarRocks unifies these into FE nodes (3 pods) + BE nodes (7 pods) = **10 pods total**. FE handles metadata coordination and Routine Load scheduling natively. Shared-data mode eliminates per-shard replication by using S3 as the source of truth.
...
| BE nodes | 7 | Shared-data mode; NVMe is data cache only; S3 is primary storage |
...
- **No per-BE replication is required** for data durability. S3 (with MinIO erasure coding or NetApp hardware redundancy) provides durability. If a BE fails, other BEs read from S3 directly; no data is lost.
...
**FE responsibilities**:
- Metadata storage and coordination (table schemas, partition metadata, BE topology, tablet assignments)
- Query parsing, planning, and fragment dispatch to BEs
- Routine Load job management: FE creates, monitors, and restarts Routine Load jobs; sub-tasks are dispatched to BEs for execution
...
- The Distributed table DDL — FE routes queries to BEs automatically; clients need no special table name
...
### 3.2 BE Nodes (Backend)

BE nodes are the **compute and data cache layer**. In shared-data mode, BEs do not own data permanently — S3 is the source of truth. The local NVMe SSD on each BE is a transparent data cache that accelerates reads for hot data.
...
**Tablet distribution**: In shared-data mode, tablets are stored in S3. Each tablet is assigned to a BE for compute purposes; FE reassigns tablets from failed BEs to healthy BEs automatically. There is no replication factor — each tablet has one S3 location, not N replicas.

**Horizontal scaling**: Adding a BE pod causes FE to redistribute tablets across all BEs automatically. No manual shard migration is required. This is a major operational improvement over ClickHouse, which requires manual resharding during scale-out (see [Section 12.2](#122-be-horizontal-scaling)).
...
| **Vault integration path** | Vault KV or dynamic credentials via MinIO admin API; Vault Agent Sidecar injects creds into BE/FE config at pod start | Vault KV for static access-key storage; Vault Agent Sidecar injection pattern applies |
...
Routine Load is the StarRocks mechanism for continuous Kafka consumption. The FE creates and manages the job; BEs execute sub-tasks.

```
Kafka Topic (N partitions)
  │
  ├─► FE (Routine Load job manager)
  │     ├─► Sub-task 0 → BE-x  (partitions 0..N/K-1)
  │     │     └─► rowsets written to S3 → Primary Key table
  │     ├─► Sub-task 1 → BE-y  (partitions N/K..2N/K-1)
  │     │     └─► rowsets written to S3 → Primary Key table
  │     └─► ... (K sub-tasks total)
  │
...
Each sub-task is assigned to a BE by FE. The BE consumes the assigned Kafka partition range for one batch interval, writes rowsets to S3, and reports completion to FE. FE commits Kafka offsets internally on success.
...
StarRocks Pipe (3.2+) is the equivalent of ClickHouse's S3Queue. The FE monitors a configured S3 path and dispatches load tasks to BEs as new files appear.

```
S3 Source Bucket
  │
  ├─► FE (Pipe job monitor)
  │     ├─► Detects new file → dispatches load task → BE-x
  │     │     └─► BE reads file from S3, writes rowsets → Primary Key table
  ...
  └─► K8s Service (ClusterIP or LoadBalancer, port 9030/8030)
        └─► Any FE node (round-robin or random pod selection)
              └─► FE parses query, generates execution plan
                    └─► FE dispatches fragments to BEs
                          ├─► BE-0 scans assigned tablets → partial result
                          ├─► BE-1 scans assigned tablets → partial result
                          │   ...
                          └─► BE-6 scans assigned tablets → partial result
                                └─► FE collects and merges → Client
```

Hot data is served from the local NVMe data cache on each BE. On a cache miss, the tablet pages are read from S3 and populated into the NVMe cache for subsequent reads.
...
- BE compute isolation between ingest (Routine Load execution) and query (fragment execution) is managed by StarRocks's internal resource groups and pipeline execution engine
- If BE compute contention between ingest and query becomes measurable, StarRocks Resource Groups (`CREATE RESOURCE GROUP`) provide workload isolation without adding a separate cluster tier

The PoC should measure CPU and memory headroom on BEs during concurrent query workloads while Routine Load sub-tasks are active. If contention is observed, configure Resource Groups before considering infrastructure changes.
...
In shared-data mode, each BE node has a two-layer storage model:
...
NVMe/SSD Data Cache  (per-BE — transparent acceleration layer)
  └──► /path/to/datacache/  [NVMe PVC on BE pod]
...
| `storage_root_path` | `/path/to/storage` | BE local working directory (not primary storage — used for intermediate state) |

S3 backend configuration (set in FE + BE via `aws_s3_*` properties or config):
...
- **Cold data is evicted**: Least-recently-used pages are evicted when cache is full; evicted data is still readable — BE transparently fetches from S3 on cache miss
...
SELECT
    ...
    datacache_disk_size,
    datacache_disk_used,
    round(datacache_disk_used / datacache_disk_size * 100, 1) AS disk_usage_pct
FROM information_schema.be_configs
WHERE name LIKE 'datacache%';

-- Cache hit metrics (StarRocks 3.x)
...
In shared-data mode, **data durability is provided entirely by S3**. There is no per-BE data replication, no replica log, and no replication coordination. The HA model for data is:
...
- Existing query fragments already dispatched to BEs continue executing
- In-flight Routine Load sub-tasks on BEs continue executing; they report to FE on completion
...
### 6.3 BE Failure and Recovery

| Failed Component | Impact | Recovery |
|---|---|---|
| 1 BE node | FE reassigns affected tablets to remaining BEs; those BEs read affected tablet data from S3 (cold cache for those tablets); no data loss | Pod restart → FE marks BE active → BE begins warming NVMe cache from S3 reads |
| Multiple BE nodes | Affected tablets' compute reassigned; cold reads from S3 for those tablets | Same — each restarted BE warms its cache independently |
| All BE nodes | All queries fail (no compute available) | Restore BEs; data is safe in S3; full cache warmup required on restart |
...
**Common requirement**: The S3 endpoint must be reachable from all 7 BE pods and all 3 FE pods on the configured port. Validate Kubernetes NetworkPolicy allows this traffic.
...
Kafka partition range
  └─► Routine Load sub-task on BE-x (assigned by FE)
        └─► BE consumes rows from Kafka
...
**Required Kubernetes NetworkPolicy — FE to BE**:
- FE pods must reach BE pods on:
  - Port **9060** (BE thrift port — heartbeat, tablet info)
  - Port **8040** (BE HTTP port — result fetching)
  - Port **9050** (BE heartbeat service port)
...
- All FE and BE pods must reach the S3 endpoint on the configured port (typically 443 or 9000 for MinIO)
...
|---|---|---|
| S3 access key + secret | FE and BE pods (shared-data S3 access) | `secret/starrocks/s3/credentials` |
...
Vault Agent runs as an init container and writes rendered secret templates into a shared volume before the FE or BE container starts. The rendered output is a configuration drop-in file.
...
1. Whether `kill -HUP` on FE/BE triggers credential re-read
...
**Mitigation**:
- PoC must sustain full 57,870 rows/sec ingest rate for ≥2 hours and measure BE CPU, memory, and write latency
...
- Tune `write_buffer_size` and compaction thread pool on BE
...
**Risk**: Routine Load fires sub-tasks on `max_batch_interval` seconds. If `max_batch_rows` is too small relative to ingest rate, each sub-task flush writes small rowsets to S3. BE nodes accumulate many small rowsets per partition, degrading query performance and increasing compaction pressure.
...
**Mitigation**:
- Tune `desired_concurrent_number` to match Kafka partition count and BE availability
...
- Validate: FE rolling upgrade (no quorum loss), BE rolling upgrade (no data unavailability), secret injection from Vault Agent Sidecar, pod anti-affinity enforcement
...
**Risk**: Physical S3 byte removal for user-ID deletes requires compaction. `ALTER TABLE t COMPACT` forces compaction but is I/O intensive and contends with query and ingest workloads on the same BE nodes.
...
**Improvement over ClickHouse**: StarRocks compaction operates at the **tablet level** (finer granularity than ClickHouse's partition-level rewrite). Tablets within a partition can compact concurrently across multiple BE nodes. This makes compaction generally faster and more parallelizable than ClickHouse's `OPTIMIZE TABLE PARTITION FINAL`, which rewrites each partition serially on a single replica. However, PoC measurement at production partition sizes is required before this advantage can be quantified.
...
|---|---|---|---|
| 1 | S3 backend: MinIO or NetApp | Storage config, BE YAML | Decision #1 (identical) |
...
| 7 | NVMe data cache sizing: `datacache_disk_size` per BE | BE pod sizing | Decision #8 (row size → cache sizing) |
| 8 | Bucket count per table partition: `BUCKETS N` in CREATE TABLE. Rule of thumb: 3× BE count = 21 for 7 BEs. Tune based on partition data size | Table DDL | No direct equivalent (CH uses shards) |
| 9 | Routine Load parallelism: `desired_concurrent_number` — how many concurrent sub-tasks? Must balance Kafka partition count and BE resource availability | Routine Load DDL | Decision #10 (Kafka partition count drives this) |
...
### Phase 2 — BE PoC (1 BE, Shared-Data)
- Deploy 1 BE pod; configure `datacache_enable = true` and `datacache_disk_path`
...
### Phase 5 — Scale BE to 7 Nodes
- Add remaining 6 BE pods; observe FE tablet redistribution
- Validate no data loss or query errors during scale-out
- Run full ingest + query workload; measure resource utilization across all BEs
...
**Current capacity**: 7 BEs × Routine Load throughput = ~57,870 rows/sec target. Routine Load parallelism is bounded by Kafka partition count and `desired_concurrent_number`.

**Scale-out path**: Add BEs (see [Section 12.2](#122-be-horizontal-scaling)) to increase Routine Load sub-task execution capacity. Increase `desired_concurrent_number` if Kafka partition count allows. Kafka partition count is the ceiling for Routine Load parallelism.
...
### 12.2 BE Horizontal Scaling

> **Improvement over ClickHouse**: ClickHouse scale-out requires adding new shard(s) and manually migrating data from existing shards — a multi-hour operation that may require downtime or careful coordination. StarRocks shared-data mode scales out by adding BE pods; FE automatically redistributes tablets to the new BEs. No manual data migration is required.

**Scale-out procedure**:
1. Add a new BE pod via kube-starrocks operator (update BE replicas in StarRocksCluster CR)
2. FE detects new BE and begins reassigning tablets to it
3. Tablet redistribution is transparent to queries and ingest — the system continues operating during rebalance
4. New BE begins warming its NVMe data cache as tablets are accessed

**Scale limit**: Shared-data mode scales to tens of BE nodes. The practical upper limit is governed by FE metadata capacity (tablet count per partition × partition count × table count) and S3 throughput.
...
FE handles concurrent query planning. BE handles concurrent fragment execution. Resource Groups can limit the resources allocated to Routine Load vs query workloads to prevent starvation:
...
|---|---|---|---|
| ≤5 | ≤5 × retention_days | Parallelizable across BEs | Feasible if per-partition compaction ≤ 24h / partition_count |
...
**Monitoring**: StarRocks exposes Prometheus metrics from both FE (port 8030/metrics) and BE (port 8040/metrics). Standard Kubernetes monitoring stacks (Prometheus + Grafana) apply directly.

**Log management**: FE and BE write structured logs. Route to the organizational log aggregation platform via standard Kubernetes log forwarding.

**Upgrade path**: kube-starrocks operator rolling upgrades. Validate in staging before production. FE upgrades first (leader → followers), then BE rolling upgrade.

### 12.8 Scalability Summary

| Dimension | Current capacity | Scale-out path | ClickHouse equivalent |
|---|---|---|---|
| Ingest rate | ~57,870 rows/sec (7 BEs) | Add BEs; increase Routine Load parallelism | Add ingest shard pods |
...
| BE compute | 7 BEs | Add BEs; automatic tablet redistribution | Add shards (manual migration required) |
| Compliance delete | Compaction per partition | Parallelize across BEs; Resource Groups | `OPTIMIZE PARTITION FINAL` per replica |
...
DISTRIBUTED BY HASH(event_id) BUCKETS 21   -- 3 × 7 BEs; see Open Decision #8
ORDER BY (event_id)
PROPERTIES (
...
### BE Configuration (`be.conf` key settings)
