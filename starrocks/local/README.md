...
│
├── redpanda-create-topics.sh       — create all Redpanda topics (idempotent)
├── redpanda-delete-topics.sh       — delete topics and all consumer groups
│
├── load-data-crash.sh              — download NYC crash CSV + produce to crashdata-topic
├── load-data-weather.sh            — download NOAA weather CSV + produce to weatherdata-topic
├── load-data-clicks.sh             — continuously stream synthetic site_clicks JSON events
│
├── starrocks-create-tables.sh      — create DB / tables / Routine Load jobs, verify counts
├── starrocks-drop-tables.sh        — stop Routine Load jobs + drop all quickstart tables
│
├── show-metrics.sh                 — cluster metrics overview (topology, tablets, lag, cache)
├── README.md                       — this file
...
| site_clicks (synthetic) | Generated by `load-data-clicks.sh` | continuous | — |

### Run the quickstart scripts

Run the following scripts in order after `docker-compose-up.sh`:

**1. Create Redpanda topics** (idempotent — safe to re-run):

```bash
./redpanda-create-topics.sh
```

Creates `crashdata-topic` (3 partitions), `weatherdata-topic` (3 partitions), and `site-clicks` (1 partition).

**2. Populate Redpanda with CSV data:**

```bash
./load-data-crash.sh    # downloads NYPD_Crash_Data.csv → crashdata-topic  (~96 MB)
./load-data-weather.sh  # downloads 72505394728.csv     → weatherdata-topic
```

Both scripts skip the download if the file already exists in `data/`. They can run in parallel in separate terminals.

**3. Create tables and Routine Load jobs in StarRocks:**

```bash
./starrocks-create-tables.sh
```

Creates the `quickstart` database, three tables, and three Routine Load jobs, waits 60 s for the first batch to commit, then prints row counts.
...
`site_clicks` starts at 0 — it is populated by `load-data-clicks.sh` (see Step 9).

> **Note**: if crash/weather counts show 0, the Routine Load batches are still committing — wait another 30 s and re-run the SELECT manually.

### Teardown

To drop all tables and jobs without stopping the stack:

```bash
./starrocks-drop-tables.sh   # stops Routine Load jobs, then drops tables
./redpanda-delete-topics.sh  # deletes topics and consumer groups
```

---

## Step 9 — Streaming Click Events (Routine Load JSON)

This step reproduces the [StarRocks Routine Load quickstart](https://docs.starrocks.io/docs/quick_start/routine-load/) using a synthetic click-stream generator. The flow is:

```
load-data-clicks.sh ──JSON──▶ site-clicks topic ──▶ clicks Routine Load ──▶ site_clicks table
```

### Start the click stream

```bash
./load-data-clicks.sh          # 25 msgs every 5 s (default — matches gen.py rate)
./load-data-clicks.sh 100 10   # 100 msgs every 10 s (custom batch / interval)
```

Press `Ctrl-C` to stop. The Routine Load job (`clicks`) remains running and will resume consuming if you restart the script.

### Monitor ingest

```sql
-- Connect: mysql -h 127.0.0.1 -P 9030 -u root quickstart
SHOW ROUTINE LOAD FOR clicks\G

-- Watch row count grow
SELECT COUNT(*), site FROM site_clicks GROUP BY site ORDER BY COUNT(*) DESC;
```

### Generated data schema

Each message is a JSON object with three fields:

| Field | Type | Description |
|---|---|---|
| `uid` | integer | Random user ID 1–10 000 |
| `site` | string | Weighted URL (49.8% homepage, 27.4% docs, 16.9% blog, 6.0% community) |
| `vtime` | integer | Unix timestamp with random look-back jitter up to 30 minutes |
...
SELECT site, COUNT(DISTINCT uid) AS unique_users
FROM site_clicks
GROUP BY site
ORDER BY unique_users DESC;

-- Visit volume by hour (using vtime)
SELECT FROM_UNIXTIME(vtime - MOD(vtime, 3600)) AS hour,
       COUNT(*) AS visits
FROM site_clicks
GROUP BY hour
ORDER BY hour DESC
...
## Table Schemas (quickstart database)

### `quickstart.crashdata`
...
**`quickstart.weatherdata`** — PRIMARY KEY model, distributed by `WEATHER_DATE`.
...
**`quickstart.site_clicks`** — DUPLICATE KEY model (append-only clicks), 1 bucket matching the single topic partition. Ingests JSON via the `clicks` Routine Load job:
...
resume with e.g. `RESUME ROUTINE LOAD FOR crash_load;`.
...
```bash
# 1. Create topics (idempotent)
./redpanda-create-topics.sh

# 2. Load CSV datasets into Redpanda (can run in parallel)
./load-data-crash.sh
./load-data-weather.sh

# 3. Create tables + Routine Load jobs in StarRocks
./starrocks-create-tables.sh

# 4. Start continuous click-stream (Ctrl-C to stop)
./load-data-clicks.sh

# Teardown tables without stopping the stack
...
# quickstart database directly
mysql -h 127.0.0.1 -P 9030 -u root quickstart
```
...
-- Individual jobs (run inside quickstart database)
SHOW ROUTINE LOAD FOR crash_load\G
SHOW ROUTINE LOAD FOR weather_load\G
SHOW ROUTINE LOAD FOR clicks\G

-- Pause / resume
PAUSE  ROUTINE LOAD FOR clicks;
RESUME ROUTINE LOAD FOR clicks;
```
...
# Consumer groups (StarRocks creates one per Routine Load job)
